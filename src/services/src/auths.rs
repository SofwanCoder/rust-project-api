use crate::{
    contracts::{GrantType, LoginUserContract},
    helpers,
...
use repositories::{
    auth::AuthRepository,
    contracts::CreateAuthRepositoryContract,
    user::UserRepository,
};
...
#[instrument(skip_all)]
pub async fn logout_a_user(db: &ApplicationDatabase, session_id: Uuid) -> Result<(), AppError> {
    let connection = &mut db.source.get_connection().await?;
    debug!("Finding auth session by id");
    AuthRepository::find_auth_by_id(connection, session_id)
        .await
        .ok_or_else(|| {
    ...
    AuthRepository::delete_auth_by_id(connection, session_id).await?;
